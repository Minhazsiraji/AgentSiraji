...
export async function POST(request: Request) {
  ...
  try {
    ...
    let body: unknown;
    try { body = JSON.parse(rawBody); } catch { return json({ error: "Invalid JSON payload." }, 400); }
    if (!body || typeof body !== "object") return json({ error: "Invalid manual payment submission." }, 400);
    const data = body as Record<string, unknown>;
    const missing = requiredFields.filter((field) => {
      const value = data[field];
      return value === undefined || value === null || String(value).trim() === "";
    });
    if (missing.length) return json({ error: "Please complete every required payment field." }, 400);

    const paymentId = String(data.paymentId).trim();
    if (!uuidPattern.test(paymentId)) return json({ error: "The payment reference is invalid." }, 400);
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(paymentDate)) return json({ error: "Payment date must use YYYY-MM-DD." }, 400);
    ...
    if (data.proofUrl) {
      ...
      } catch { return json({ error: "Proof link is invalid." }, 400); }
    }
    ...
    console.error("Subscription manual payment submission failed", error);
