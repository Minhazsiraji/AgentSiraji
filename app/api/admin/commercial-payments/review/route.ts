...
export async function POST(request: Request) {
  ...
  if (!admin) return json({ error: "Unauthorized manual-payment review." }, 401);
  ...
  try {
    ...
    if (!uuidPattern.test(paymentId) || !["APPROVED", "REJECTED", "NEEDS_INFORMATION"].includes(decision)) return json({ error: "A valid payment reference and review decision are required." }, 400);
    if (reviewNote.length > 1_000) return json({ error: "Review note is too long." }, 400);
