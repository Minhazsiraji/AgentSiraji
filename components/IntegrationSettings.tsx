...
export function IntegrationSettings() {
  ...
  return <div style={{ display: "grid", gap: 22 }}>
    ...
    <section className="glass-card" style={{ padding: 24 }}><h2>Meta Pixel + CAPI</h2><p>Use only the AgentSiraji Pixel and its Conversions API token. The health check can send one synthetic PageView to Meta Test Events using your test code; Purchase events remain disabled.</p><div className="form-row">{input("metaPixelId", "Pixel ID", false, "1234567890")}{input("metaTestEventCode", "Test Event Code", true, "TEST12345")}</div>{input("metaCapiAccessToken", "Conversions API access token", true, "Server-side secret")}</section>
    <section className="glass-card" style={{ padding: 24 }}><h2>Google tag</h2><p>Store the AgentSiraji Google tag ID. Supported public IDs start with G-, AW- or GT-. The browser tag uses the ID only after measurement consent. The optional API secret is not required for normal browser tagging and remains server-side if you later use Measurement Protocol.</p><div className="form-row">{input("googleMeasurementId", "Google tag / measurement ID", false, "G-XXXXXXXXXX, AW-XXXXXXXXX or GT-XXXXXXX")}{input("googleApiSecret", "Google API secret (optional)", true, "Leave blank for browser-only tagging")}</div></section>
