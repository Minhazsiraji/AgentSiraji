...
export function IntegrationSettings() {
  ...
  const [message, setMessage] = useState("Platform admin session required. Add values only when configuring or replacing a setting.");
  ...
  async function request(method: "GET" | "PUT") {
    const response = await fetch("/api/admin/integrations", { method, headers: method === "PUT" ? { "content-type": "application/json" } : undefined, body: method === "PUT" ? JSON.stringify(values) : undefined, cache: "no-store" });
  ...
  return <div style={{ display: "grid", gap: 22 }}>
    <p>Leave credential fields blank to keep their saved encrypted values. Enter a value only when adding or replacing a setting. Access is controlled by your signed-in platform admin session.</p>
    ...
    <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}><button className="button button-primary" type="button" onClick={save} disabled={busy}>{busy ? "Working…" : "Save & check connections"}</button><button className="button button-secondary" type="button" onClick={check} disabled={busy}>Refresh server health</button><button className="button button-secondary" type="button" onClick={runBrowserChecks} disabled={browserBusy}>{browserBusy ? "Checking browser…" : "Run browser delivery checks"}</button></div>
